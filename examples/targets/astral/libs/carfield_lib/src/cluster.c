...
int handle_dma_transfer(
    ...
    mini_printf("[PULP][DMA] DMA Transfer: %s(%p) %s %s(%p) - Tensor type: %s\r\n",
        ...
        tensor->tensor_type == MATCH_VAR_TENSOR ? "VAR" : (tensor->tensor_type == MATCH_CONST_TENSOR ? "CONST" : "OUT"));
    ...
    switch(tensor->num_dims){
        ...
        case 4: {
            int is_hwc_to_chw = (ctx->pattern_name==depthwise_conv2d && tensor->tensor_type==MATCH_VAR_TENSOR && ctx->exec_module==PULP_CLUSTER);
...
void cluster_wait_for_task_mbox(volatile uint32_t** tensor_ptrs, volatile uint32_t* task_id) {
    ...
    asm volatile("fence rw,rw" ::: "memory");
    #else
    cluster_wait_for_task_poll(tensor_ptrs, task_id);
    #endif
}
...
void cluster_end_of_task_mbox(uint32_t task_id) {
    ...
    asm volatile("fence rw,rw" ::: "memory");
    #else
    cluster_end_of_task_poll(task_id);
    #endif
}
