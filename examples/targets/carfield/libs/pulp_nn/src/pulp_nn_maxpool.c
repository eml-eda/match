  ...
  uint16_t  stride_y)
{
    ...
    int n_cores = get_core_num();
    if (dim_im_in_y < get_core_num())
    {
      n_cores = dim_im_in_y;
  }
  int  Log2Core = log2(n_cores);

  int chunck = (dim_im_in_y >> Log2Core) + ((dim_im_in_y & (get_core_num()-1))!=0);
  ...
  for (i_y = start; i_y < stop; i_y++)
  {
    ...
    synch_barrier();
    if (dim_im_out_y < get_core_num())
    {
        n_cores = dim_im_out_y;
    }
    Log2Core = log2(n_cores);
    int chunck2 = (dim_im_out_y >> Log2Core) + ((dim_im_out_y & (get_core_num()-1))!=0);
