...
int TVMPlatformMemoryAllocate(size_t num_bytes, DLDevice dev, void** out_ptr) {
  ...
  // mini_printf("[TVM Platform] Allocating %d bytes of memory\n\r", num_bytes);
  *out_ptr = match_try_alloc_from_match_mem(num_bytes);
  if (*out_ptr == NULL){
      // mini_printf("[TVM Platform] Failed to allocate memory\n\r");
      return 1;
  }
  // mini_printf("[TVM Platform] Successfully allocated memory at %p\n\r", *out_ptr);
  return 0;
}

int TVMPlatformMemoryFree(void* ptr, DLDevice dev) {
  // free(ptr);
  // mini_printf("[TVM Platform] Freeing memory at %p\n\r", ptr);
  match_try_free_from_match_mem(ptr);
  return 0;
}
