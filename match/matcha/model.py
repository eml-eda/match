...
class MatchaModel:
    ...
    def set_model(out_path, model_name, executor):
        ...
        reset_relay_list()
        reset_schedules()
        model_path = MatchaModel.get_path(out_path=out_path, model_name=model_name)
    ...
    def compile(self, target, out_path):
        ...
        if len(self.other_models) > 0:
            for model_name, model in self.other_models.items():
                model_path = MatchaModel.set_model(out_path=out_path, model_name=model_name, executor=self.executor)
                ...
                MatchaModel.save_model_logs()
                MatchaModel.gen_model_runtime_and_move_model(
                    target=target,
                    out_path=out_path,
        ...
        if self.golden_cpu_model:
            ...
            model_path = MatchaModel.set_model(out_path=out_path, model_name=self.model_name + "_golden_cpu", executor="aot")
        ...
        # compile the model
        model_path = MatchaModel.set_model(out_path=out_path, model_name=self.model_name, executor=self.executor)
        comp = self.compilation_cls(
            self.relay_mod, self.relay_params, target=target, build_dir=model_path, mod_name=self.model_name
        )
        comp.tvm_compile(fusion=False)
        MatchaModel.save_model_logs()
        print("[MODEL COMPILE] Generating runtime and moving files...")
        MatchaModel.gen_model_runtime_and_move_model(
            target=target,
            out_path=out_path,
        ...
        print("[MODEL COMPILE] Completed successfully.")
    ...
    def model_runtime(self, target=None, out_path: str = "/build"):
        ...
        if not Path(abs_out_path + "/runtime").is_dir():
            subprocess.getoutput(
                f"unzip {Path(os.path.dirname(__file__) + '/../model/tvm_runtime.zip').absolute()} -d {Path(abs_out_path + '/')}"
            )
        
        runtime_src = Path(abs_out_path + f"/src/{self.model_name}/runtime.c")
        runtime_src.parent.mkdir(parents=True, exist_ok=True)
        template_file = os.path.dirname(__file__) + "/../libs/c/mako/match/src/runtime.c"
        runtime_src.write_text(Template(filename=template_file).render(**temp_args))
        
        runtime_include = Path(abs_out_path + f"/include/{self.model_name}/runtime.h")
        runtime_include.parent.mkdir(parents=True, exist_ok=True)
        template_file = os.path.dirname(__file__) + "/../libs/c/mako/match/include/runtime.h"
        runtime_include.write_text(Template(filename=template_file).render(**temp_args))
    ...
    def gen_model_runtime_and_move_model(
        ...
        build_dir = MatchaModel.get_path(out_path=out_path, model_name=model_name)
        abs_out_path = str(Path(out_path).absolute())
        ...
        if executor == "graph":
            ...
            try:
                ...
                with open(f"{build_dir}/codegen/host/src/{model_name}_graph.c", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/matcha/graph.c"
                ...
                with open(f"{build_dir}/codegen/host/include/{model_name}_graph.h", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/matcha/graph.h"
                ...
                with open(f"{build_dir}/codegen/host/src/{model_name}_params_data.c", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/matcha/params_data.c"
                    content = Template(filename=filename).render(**graph_runtime_template_data)
                    run_file.write(format_c_code(content))
                with open(f"{build_dir}/codegen/host/include/{model_name}_params_data.h", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/matcha/params_data.h"
                    content = Template(filename=filename).render(**graph_runtime_template_data)
                    run_file.write(format_c_code(content))
            except Exception as e:
                print("[TEMPLATE WRITER] Error processing graph runtime template")
                with open("mako_error_dump.html", "wb") as output_file:
