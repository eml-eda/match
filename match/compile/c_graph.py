...
def match_tvmc_graph_compile_wrapper(
    ...
    """
    These are the existing configurations: 
        tir.ReduceBranchingThroughOvercompute, 
        tir.experimental_dma_bypass_cache,
        tir.reset_start_id, 
    ...
    # remove the archive
    os.remove(mlf_path)
    print(f"[TVM] Compilation complete. Build Path: {build_path}")
...
class MatchCompilerCGraph(MatchCompiler):
    ...
    def tvm_compile(self, target_additional_options: Dict[str, str] = {"requant_transform": "0"}, fusion: bool = True):
        target_options = ""
        for key, val in target_additional_options.items():
            target_options += f"-{key}={val} "
        match_tvmc_graph_compile_wrapper(
            self.model,
