...
def get_pattern_length(pattern):
    if isinstance(pattern, CallPattern):
        return 1 + get_pattern_length(pattern.args[0])
    elif isinstance(pattern, AttrPattern):
        return get_pattern_length(pattern.pattern)
    elif isinstance(pattern, AltPattern):
        return max(get_pattern_length(pattern.left), get_pattern_length(pattern.right))
    return 0
...
class MatchTargetPattern:
    ...
    def __init__(
        ...
        module_pattern,
        name: str = "conv2d",
        original_name: str = "conv2d",
        match_additional_checks=mock_func,
        idx=0,
    ):
...
class MatchTarget(ABC):
    ...
    def __init__(self, exec_modules, name: str = "match", optimize_param: str = "latency", **kwargs):
        ...
        self.model_generative_apis_include_path = (
            os.path.dirname(__file__) + "/../libs/c/mako/default/include/generative_model_apis.h"
        )
    ...
    def get_layer_from_module(self, mod: tvm.ir.IRModule, exec_module_name: str = "", pattern_name: str = "conv2d"):
        ...
        if schedule is None:
            ...
            schedule.latency = schedule_gen.latency
            schedule.energy = schedule_gen.energy
    ...
    def match_additional_checks_(self, node, match_pt: MatchTargetPattern = None):
        ...
        if not isinstance(node, tvm.relay.Call):
            print("[PATTERN MATCHER] Node is not a call, weird behaviour")
                ...
                if is_pattern_matching(other_pt.pattern(), node):
                    ...
                    if other_pt.additional_checks(node_add_checks):
                        try:
                            ...
                            print(
                                f"[PATTERN MATCHER] Node is also supported by {other_pt.name} with expected latency {other_pt_latency} and expected energy {other_pt_energy}\n"
                            )
                        except Exception as exc:
                        ...
                        if self.is_better_result(latency, energy, other_pt_latency, other_pt_energy):
                            print(
                                f"[PATTERN MATCHER] Pattern {other_pt.name} is expected to be better for this node than {match_pt.name}, refusing current pattern"
                            )
                            return False
                        else:
                            print(
                                f"[PATTERN MATCHER] Pattern {match_pt.name} is expected to be better for this node than {other_pt.name}, checking remaining patterns"
                            )
                    else:
    ...
    def sort_match_patterns(self):
        ...
        self.match_patterns = sorted(self.match_patterns, key=lambda m_pt: -get_pattern_length(m_pt.pattern()))
    ...
    def add_exec_module(self, exec_module):
        ...
        for module_pt in exec_module.partitioning_patterns():
            match_pt = MatchTargetPattern(
                exec_module, module_pt, name=f"{self.name}.{exec_module.name}.{module_pt.name}", original_name=module_pt.name
            )
...
class DefaultMatchTarget(MatchTarget):
    def __init__(self):
        super(DefaultMatchTarget, self).__init__([], name="default")
