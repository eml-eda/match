...
def check_if_any_valid_allocation(
    ...
    if len_at_time > 1:
        for tens_a, tens_b in zip(
            tensors_allocated_at_time[time][: len_at_time - 1],
            tensors_allocated_at_time[time][1:],
        ):
...
def check_if_valid_allocation(
    ...
    if len_at_time > 1:
        for tens_a, tens_b in zip(
            ...
            if start_tens_b - end_tens_a >= tens_size:
                if (
                    start_tens_b >= offset >= end_tens_a
                    and start_tens_b >= offset + tens_size >= end_tens_a
                ):
                    return True
        ...
        if available_soc_bytes - end_tens >= tens_size:
            if (
                available_soc_bytes >= offset >= end_tens
                and available_soc_bytes >= offset + tens_size >= end_tens
            ):
                return True
    elif len_at_time == 1:
        # check if tensor can be allocated before
    ...
    if valid_for_all:
        ...
        for time in range(tensor.start_usage, tensor.last_usage + 1):
            if time in calls_idxs:
                ...
                tensors_allocated_at_time[time] = sorted(
                    tensors_allocated_at_time[time] + [tensor],
                    key=lambda m_t: m_t.mem_offset_at[time],
                )
        allocated = True
        ...
        for time in range(tensor.start_usage, tensor.last_usage + 1):
            if time in calls_idxs:
                if time in tensor.used_at:
                    ...
                    if loaded_first_time and any(
                        [time > use_time for use_time in tensor.used_at]
                    ):
        ...
        if tensor.is_output and tensor_fixed_to_ext_mem:
            last_usage = tensor.used_at[-1]
            if not any(
                [move_time > last_usage for move_time in tensor.move_temp_to_ext_mem]
            ):
                for call_idx in calls_idxs:
                    if call_idx > last_usage:
                        tensor.move_temp_to_ext_mem.append(call_idx)
        ...
        if tensor.is_output:
            print(
                f"[MEMORY PLANNER] Tensor {tensor.name} is an output tensor, it will be moved to external memory at {tensor.move_temp_to_ext_mem}"
            )
    return allocated
...
def try_allocate_congested(
    ...
    allocated = False
    for tens_a, tens_b in zip(
        tensors_allocated_at_time[time][: len(tensors_allocated_at_time[time]) - 1],
        tensors_allocated_at_time[time][1:],
    ):
        end_tens_a = tens_a.mem_offset_at[time] + tens_a.num_bytes
        start_tens_b = tens_b.mem_offset_at[time]
...
def allocate_tensor(
    ...
    for time in range(tensor.start_usage, tensor.last_usage + 1):
        if time in calls_idxs and time in tensor.used_at:
            if max_allocated_tensors_at == -1 or len(
                tensors_allocated_at_time[time]
            ) > len(tensors_allocated_at_time[max_allocated_tensors_at]):
                max_allocated_tensors_at = time
            if (
                less_free_mem_at == -1
                or free_size_at_time[time] < free_size_at_time[less_free_mem_at]
            ):
    ...
    if not allocated:
        print(
            f"[MEMORY PLANNER] Couldnt allocate all the tensors, tensor {tensor.name} allocation was not successfull"
        )
        print(
            f"[MEMORY PLANNER] Node at {max_allocated_tensors_at} cannot fit SoC memory"
        )
        # TODO: add list of nodes to run from external memory
        raise Exception(
            f"[MEMORY PLANNER] Couldnt allocate all the tensors, tensor {tensor.name} with size {tens_size} allocation was not successfull"
        )
