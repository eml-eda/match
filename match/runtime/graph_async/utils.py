...
def save_memory_allocation_graph(mem_tensors, available_soc_bytes: int = 0, output_file="memory_allocation.png"):
    """
    ...
    for tensor in mem_tensors:
        ...
        if abbreviate_name(tensor.name) not in [handle.get_label() for handle in legend_handles]:
            legend_handles.append(mpatches.Patch(color=color, label=f"{abbreviate_name(tensor.name)}: {tensor.name}"))
        for time, offset in tensor.mem_offset_at.items():
            ...
            if time in tensor.load_from_ext_mem_at:
                ax.annotate(
                    "",
                    xy=(time + 0.5, offset + height),
                    xytext=(time + 0.5, offset + height + 0.1),
            ...
            if time in tensor.move_temp_to_ext_mem:
                ax.annotate(
                    "",
                    xy=(time + 0.5, offset + height + 0.1),
                    xytext=(time + 0.5, offset + height),
    ...
    ax.set_ylim(0, max([max(list(tensor.mem_offset_at.values()) + [0]) + tensor.num_bytes for tensor in mem_tensors] + [0]) + 32)
    ax.grid(True)
    ...
    print("[MEM PLANNER] Memory allocation graph saved")
