...
class ZigZagMatchCostModel(CostModelEvaluation):
    ...
    def __init__(
        ...
        *,
        ...
        constrained_temporal_mapping = TemporalMapping(
            temporal_mapping_dict=constrained_temporal_mapping_dict, layer_node=temporal_mapping.layer_node
        )
    ...
    def calc_relevancy_map(self):
        ...
        batch_matmul = "batch_matmul" in self.match_node.ops_occurrences
        ...
        if dense:
            ...
            ordered_relevant_loops = {"X": [], "Y": [], "W": [], "I": [], "O": []}
            ...
            operand: {
                reldim: [
                    prod(
                        [
                            val[1]
                            for m_lev in range(memory_level + 1)
                            for val in self.temp_mapping[operand][m_lev]
                            if val[0] == reldim
                        ]
                        ...
                        + [
                            strides[
                                0
                                if (reldim == "OY" and stride_length < 3) or reldim == "OD"
                                else 1
                                if (reldim == "OY" and stride_length > 2) or (reldim == "OX" and stride_length < 3)
                                else 2
                            ]
                            if operand in self.input_operands and reldim in ["OD", "OY", "OX"]
                            else 1
                        ]
                    )
