...
class MatchConv2dPWStridedRewriter(DFPatternCallback):
    def __init__(self, require_type=False):
        super().__init__(require_type)
        self.data = wildcard()
    ...
    def callback(self, pre, post, node_map):
        ...
        data_nb = relay.op.take(data_shape, relay.const(0))
        data_c = relay.op.take(data_shape, relay.const(1))
        data_h = relay.op.take(data_shape, relay.const(2))
        data_w = relay.op.take(data_shape, relay.const(3))
...
class MatchConv2dPWStrided:
    ...
    def transform_module(
        ...
        for global_var, func in mod.functions.items():
            func = rewrite(MatchConv2dPWStridedRewriter(), func)
