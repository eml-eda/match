...
def schedule_to_code(
    mod: tvm.ir.IRModule, exec_module_name: str = "", pattern_name: str = ""
):
    target = get_target()
    schedule, match_node, exec_module, latency, energy = target.get_layer_from_module(
        mod=mod, exec_module_name=exec_module_name, pattern_name=pattern_name
    )
    tempengine = TemplateWriter(
        mod=mod,
        target=target,
...
def codegen(mod: tvm.ir.IRModule):
    ...
    try:
        code = schedule_to_code(
            mod=mod, 
            exec_module_name=exec_module_name, 
            pattern_name=pattern_name
        )
        error_codegen = False
