...
class MatchSaveModule:
    def transform_module(
        ...
        match_inputs=[{"name":inp_.name_hint,"size":prod(inp_.type_annotation.shape[1:]),"type":inp_.type_annotation.dtype,"prec":np.dtype(inp_.type.annotation.dtype).itemsize,"shape":[int(sh) for sh in inp_.type_annotation.shape]} for inp_ in relay_inputs]
