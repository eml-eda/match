...
class MatchSaveModule:
    def transform_module(
        ...
        match_inputs=[{"name":inp_.name_hint,"size":prod(inp_.type_annotation.shape[1:]),"type":inp_.type_annotation.dtype,"shape":[int(sh) for sh in inp_.type_annotation.shape]} for inp_ in relay_inputs]
        match_output={"size":prod(func.ret_type.shape[1:]),"type":func.ret_type.dtype,"shape":[int(sh) for sh in func.ret_type.shape]}
